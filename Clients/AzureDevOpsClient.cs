...
internal sealed class AzureDevOpsClient(HttpClient client, AzureDevOpsProjectReference project) : IDisposable
{
	...
	#region Properties
	private string ProjectApiRoot => $"{project.CollectionUrl}/{Uri.EscapeDataString(project.Project)}/_apis/git";
	#endregion

	#region Publics
	public async Task<AzureProjectInfo> GetProjectAsync(CancellationToken cancellationToken)
	{
		string strUrl = $"{project.CollectionUrl}/_apis/projects/" + $"{Uri.EscapeDataString(project.Project)}?api-version={API_VERSION}";

		using HttpRequestMessage request = RestSupport.Json(HttpMethod.Get, strUrl);

		using JsonDocument? document = await RestSupport.SendAllowingNotFoundAsync(client,
		                                                                           request,
		                                                                           SERVICE_NAME,
		...
		return new AzureProjectInfo(RestSupport.RequiredString(root, "id", SERVICE_NAME), RestSupport.StringOrNull(root, "name") ?? project.Project);
	}
	...
	public async Task<AzureRepositoryInfo?> FindRepositoryAsync(string strName, CancellationToken cancellationToken)
	{
		...
		using JsonDocument? document = await RestSupport.SendAllowingNotFoundAsync(client, request, SERVICE_NAME, ExitCode.TargetError, cancellationToken);

		return document is null ? null : ReadRepository(document.RootElement);
	}

	public async Task<AzureRepositoryInfo> CreateRepositoryAsync(string strName, string strProjectId, CancellationToken cancellationToken)
	{
		string strUrl = $"{this.ProjectApiRoot}/repositories?api-version={API_VERSION}";

		// Name the properties explicitly: an anonymous type infers them from the variable,
		// so a rename would silently change the JSON the API receives.
		using HttpRequestMessage request = RestSupport.Json(HttpMethod.Post, strUrl, new { name = strName, project = new { id = strProjectId } });

		using JsonDocument document = await RestSupport.SendAsync(client, request, SERVICE_NAME, ExitCode.TargetError, cancellationToken);
	...
	{
		string strUrl = $"{this.ProjectApiRoot}/repositories/{Uri.EscapeDataString(strRepositoryId)}" + $"/refs?api-version={API_VERSION}&$top=10000";

		using HttpRequestMessage request = RestSupport.Json(HttpMethod.Get, strUrl);

		using JsonDocument document = await RestSupport.SendAsync(client, request, SERVICE_NAME, ExitCode.TargetError, cancellationToken);

		if(!document.RootElement.TryGetProperty("value", out JsonElement value) || value.ValueKind != JsonValueKind.Array) return [];
	...
	/// </summary>
	public async Task SetDefaultBranchAsync(string strRepositoryId, string strBranchRefName, CancellationToken cancellationToken)
	{
		string strUrl = $"{this.ProjectApiRoot}/repositories/{Uri.EscapeDataString(strRepositoryId)}?api-version={API_VERSION}";

		using HttpRequestMessage request = RestSupport.Json(HttpMethod.Patch, strUrl, new { defaultBranch = strBranchRefName });

		// The updated repository is returned; we only care that the call succeeded.
		(await RestSupport.SendAsync(client, request, SERVICE_NAME, ExitCode.TargetError, cancellationToken)).Dispose();
	}
	...
	private static AzureRepositoryInfo ReadRepository(JsonElement element)
	{
		...
		return new AzureRepositoryInfo(RestSupport.RequiredString(element, "id", SERVICE_NAME),
	...
	{
		if(!Uri.TryCreate(strUrl, UriKind.Absolute, out Uri? uri) || uri.UserInfo.Length == 0) return strUrl;
