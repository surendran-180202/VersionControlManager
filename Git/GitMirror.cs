...
internal sealed class GitMirror(GitCommandRunner git)
{
	...
	public async Task CloneAsync(string strCloneUrl, string strDestination, string strAuthorizationHeader, CancellationToken cancellationToken)
	{
		...
		{
			throw new MigrationException(ExitCode.SourceError,
	...
	{
		GitResult gitResultRefs = await git.RunAsync(["for-each-ref", "--format=%(refname)%09%(objecttype)"], strRepositoryPath, cancellationToken: cancellationToken);

		if(!gitResultRefs.Success)
		{
			throw new MigrationException(ExitCode.GitError, $"Could not list references in the mirror: {gitResultRefs.FailureText}");
		}

		GitRef[] liRefs =
		[
			.. gitResultRefs.StandardOutput
		...
		if(liRefs.Length > 0)
		{
			GitResult gitResultCount = await git.RunAsync(["rev-list", "--all", "--count"], strRepositoryPath, cancellationToken: cancellationToken);

			if(gitResultCount.Success && int.TryParse(gitResultCount.StandardOutput.Trim(), out int nParsed)) nCommitCount = nParsed;
		}
	...
	/// </summary>
	public async Task PushAsync(string strRepositoryPath, string strTargetUrl, MirrorSummary mirrorSummary, bool bIncludeNotes, string strAuthorizationHeader, CancellationToken cancellationToken)
	{
		...
		GitResult gitResult = await git.RunAsync(liArguments, strRepositoryPath, strAuthorizationHeader, bRelayProgress: true, cancellationToken: cancellationToken);

		if(!gitResult.Success)
		{
			throw new MigrationException(ExitCode.TargetError, $"Push to Azure DevOps failed.{Environment.NewLine}{ConsoleLog.Redact(gitResult.FailureText)}", DescribePushFailure(gitResult.FailureText));
		}
	}
	...
	{
		GitResult gitResult = await git.RunAsync(["cat-file", "-p", "HEAD:.gitattributes"], strRepositoryPath, cancellationToken: cancellationToken);

		// A non-zero exit just means there is no .gitattributes on the default branch.
		return gitResult.Success && gitResult.StandardOutput.Contains("filter=lfs", StringComparison.OrdinalIgnoreCase);
	}
	...
	/// </summary>
	public async Task PushLfsAsync(string strRepositoryPath, string strTargetUrl, string sourceAuthorizationHeader, string targetAuthorizationHeader, CancellationToken cancellationToken)
	{
		GitResult gitResultFetch = await git.RunAsync(["lfs", "fetch", "--all", "origin"], strRepositoryPath, sourceAuthorizationHeader, bRelayProgress: true, cancellationToken: cancellationToken);

		if(!gitResultFetch.Success)
		{
			throw new MigrationException(ExitCode.SourceError, $"Could not fetch Git LFS objects from the source.{Environment.NewLine}{ConsoleLog.Redact(gitResultFetch.FailureText)}");
		}

		GitResult gitResultPush = await git.RunAsync(["lfs", "push", "--all", strTargetUrl], strRepositoryPath, targetAuthorizationHeader, bRelayProgress: true, cancellationToken: cancellationToken);

		if(!gitResultPush.Success)
		{
			throw new MigrationException(ExitCode.TargetError, $"Could not push Git LFS objects to the target.{Environment.NewLine}{ConsoleLog.Redact(gitResultPush.FailureText)}");
		}
	}
	...
	{
		GitResult gitResult = await git.RunAsync(["symbolic-ref", "--short", "HEAD"], strRepositoryPath, cancellationToken: cancellationToken);
	...
	private static long MeasureDirectory(string strPath)
	{
		try
		{
			return new DirectoryInfo(strPath).EnumerateFiles("*", SearchOption.AllDirectories).Sum(f => f.Length);
		}
	...
	private static string? DescribeCloneFailure(string strFailureText)
	{
		...
		if(strFailureText.Contains("not found", StringComparison.OrdinalIgnoreCase) || strFailureText.Contains("404", StringComparison.Ordinal))
		{
	...
	private static string? DescribePushFailure(string strFailureText)
	{
		...
		if(strFailureText.Contains("non-fast-forward", StringComparison.OrdinalIgnoreCase) || strFailureText.Contains("fetch first", StringComparison.OrdinalIgnoreCase))
		{
		...
		if(strFailureText.Contains("VS403636", StringComparison.Ordinal) || strFailureText.Contains("too large", StringComparison.OrdinalIgnoreCase))
		{
